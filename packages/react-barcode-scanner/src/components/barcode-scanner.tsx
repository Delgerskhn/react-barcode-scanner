...
const BarcodeScanner = forwardRef<HTMLVideoElement, ScannerProps>(
  ({ options, onCapture, trackConstraints, paused = false, ...props }, ref) => {
    const instance = useRef<HTMLVideoElement>(null)
    const { isCameraReady } = useCamera(instance, trackConstraints)
    const { detectedBarcodes, startScan, stopScan } = useScanning(
      instance,
      options
    )

    useImperativeHandle(ref, () => instance.current as HTMLVideoElement, [])
